...
public class FileTrackStore implements TrackStore {
	...
	public TrackStoreEntry[] getEntries() throws TrackStoreException {
		...
		try {
			...
			while (binList.hasMoreElements()) {
				...
				FileTrackStoreEntry entry = readBinaryTrack(connection, file);
				if (entry != null) {
			...
			while (txtList.hasMoreElements()) {
				...
				FileTrackStoreEntry entry = readTextTrack(connection, file);;
				if (entry != null) {
					result.addElement(entry);
				}
			}
	...
	private FileTrackStoreEntry readTextTrack(FileConnection connection,
			final String file) {
		FileTrackStoreEntry entry = null;
		try {
			// TODO: Parse only first line
			String filename = connection.getURL() + file;
			Vector list = ConfigFile.openList(filename);
	...
	private boolean isValidChar(final char ch) {
		...
		return Character.isDigit(ch) || Character.isLowerCase(ch) || Character.isUpperCase(ch) || valid.indexOf(ch) != -1;
	}
	
	private FileTrackStoreEntry readBinaryTrack(FileConnection connection,
			final String file) {
	...
	public void saveTrack(final Track track) throws TrackStoreException {
		...
		try {
			connection = FileUtil.createFile(dir, track.getName(), 
					BBT_EXTENSION);
	...
	private class FileTrackStoreEntry extends TrackStoreEntry {
		...
		private Track loadTextTrack() throws TrackStoreException {
			try {
				Vector list = ConfigFile.openList(url);
